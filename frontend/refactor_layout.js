...
const leftColRegex = /\{\/\* COLUMN 1: LEFT SIDEBAR.*?<div className="lg:col-span-3 space-y-5">\s*<\/div>/s;
...
if (rightColEnd === -1) {
  console.log("Could not find right column end");
  process.exit(1);
}

// The end of right column is:
//           </div>
//         </div>
//       </div>
//
//       {/* ─── Floating Action Button 
// We want to insert parcelIntelligenceCode just before the last </div>
const insertionPoint = content.lastIndexOf('</div>', rightColEnd);
content = content.substring(0, insertionPoint) + '\n          ' + parcelIntelligenceCode + content.substring(insertionPoint);
