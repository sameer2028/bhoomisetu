...
import { Layers, Eye, EyeOff, Route, SquareDashedBottom, ChevronDown, ChevronUp } from 'lucide-react';
import {
  BASE_LAYERS,
  PARCEL_STATUS_ORDER,
  formatAcres,
...
export default function MapLegend({
  ...
  return (
    <div className="absolute bottom-4 right-4 z-[1000] w-64 bg-white/95 backdrop-blur-md rounded-2xl border border-slate-200/90 shadow-floating overflow-hidden">
      <div 
        className="px-3.5 py-2.5 border-b border-slate-100 bg-slate-50/70 flex items-center justify-between cursor-pointer hover:bg-slate-100/50 transition-colors"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {isExpanded && !allActive && (
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); onShowAllStatuses(); }}
