...
function Row({ label, value, mono = false }) {
  return (
    <div className="flex items-start justify-between gap-3 py-1.5 border-b border-neutral-100 last:border-0">
      ...
      <span
        className={`text-[11.5px] font-semibold text-neutral-800 text-right ${mono ? 'font-mono' : ''}`}
      >
        {value ?? '—'}
      </span>
    </div>
...
function SectionHeading({ icon: Icon, children, tint = 'text-blue-700' }) {
  return (
    <h4 className="flex items-center gap-1.5 text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-1.5">
...
export default function ParcelDetailPanel({ feature, loading, onClose }) {
  if (loading) {
    return (
      <aside className="w-[360px] flex-shrink-0 bg-white border-l border-neutral-200 flex items-center justify-center">
        <div className="text-center">
          <span className="spinner spinner-lg mb-3" />
          <p className="text-xs text-neutral-500">Loading parcel record...</p>
        </div>
      </aside>
    );
  }
  ...
  if (!feature) {
    return (
      <aside className="w-[360px] flex-shrink-0 bg-white border-l border-neutral-200 flex items-center justify-center p-8">
        <div className="text-center">
          <div className="w-12 h-12 rounded-xl bg-neutral-50 border border-neutral-200 flex items-center justify-center mx-auto mb-3">
            <Crosshair className="w-6 h-6 text-neutral-300" />
          </div>
          <h3 className="text-sm font-bold text-neutral-800">No Parcel Selected</h3>
  ...
  return (
    <aside className="w-[360px] flex-shrink-0 bg-white border-l border-neutral-200 flex flex-col overflow-hidden slide-in">
      {/* Header */}
      <div className="px-4 py-3 border-b border-neutral-200 flex items-start justify-between gap-2 flex-shrink-0">
        ...
        <button
          onClick={onClose}
          className="p-1 rounded-md text-neutral-400 hover:bg-neutral-100 hover:text-neutral-700 transition-colors flex-shrink-0"
          title="Close panel"
        >
          <X className="w-4 h-4" />
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
        ...
        {areaDivergent && (
          <div className="flex gap-2 p-2.5 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <div className="text-[11px] leading-snug">
              <p className="font-bold text-amber-900">Area needs review</p>
        ...
        <section>
          ...
          <div className="bg-neutral-50 rounded-lg border border-neutral-100 px-3 py-1.5">
        ...
        <section>
          ...
          <p className="text-[9.5px] text-neutral-400 mt-1.5 flex items-center gap-1">
            <Database className="w-3 h-3" />
            {GEOMETRY_SOURCE_LABELS[p.geometry_source] || p.geometry_source} &middot; EPSG:4326
          </p>
        </section>
        ...
        <section>
          ...
          {p.project_code ? (
            <div className="bg-blue-50/60 rounded-lg border border-blue-100 px-3 py-2.5">
              ...
              <p className="text-[10px] text-neutral-500 mt-0.5">{p.implementing_agency}</p>
              {p.corridor_width_m && (
              ...
              <Link
                ...
                className="btn btn-secondary btn-sm w-full justify-center mt-2.5 !text-[11px]"
              >
        ...
        <section>
          ...
          <div className="space-y-2">
            {/* Compensation */}
            <div className="flex items-center gap-2.5 p-2.5 rounded-lg border border-neutral-200 bg-white">
              ...
              <div className="min-w-0 flex-1">
                ...
                {comp ? (
                  ...
                  <p className="text-xs font-semibold text-neutral-400">Not assessed yet</p>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 p-2.5 rounded-lg border border-neutral-200 bg-white">
              ...
              <div className="min-w-0 flex-1">
                ...
                {poss?.possession_date && (
                  <p className="text-[10px] text-neutral-500">Recorded {poss.possession_date}</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="p-2 rounded-lg border border-neutral-200 text-center">
                <FileText className="w-3.5 h-3.5 text-purple-600 mx-auto" />
                <p className="text-sm font-bold text-neutral-900 mt-0.5">{p.total_documents}</p>
                <p className="text-[9px] text-neutral-400 uppercase tracking-wide">Docs</p>
              </div>
              <div className="p-2 rounded-lg border border-neutral-200 text-center">
                <GitBranch className="w-3.5 h-3.5 text-blue-600 mx-auto" />
                <p className="text-sm font-bold text-neutral-900 mt-0.5">{p.total_cases}</p>
                <p className="text-[9px] text-neutral-400 uppercase tracking-wide">Cases</p>
              </div>
              <div className="p-2 rounded-lg border border-neutral-200 text-center">
                <Users className="w-3.5 h-3.5 text-amber-600 mx-auto" />
                <p className="text-sm font-bold text-neutral-900 mt-0.5">{p.total_families}</p>
                <p className="text-[9px] text-neutral-400 uppercase tracking-wide">Families</p>
              </div>
            </div>
          </div>
        </section>
        ...
        {p.documents?.length > 0 && (
          <section>
            <SectionHeading icon={FileText} tint="text-purple-700">
              Attached Documents
            </SectionHeading>
            <ul className="space-y-1">
              {p.documents.map((doc) => (
                <li
                  key={doc.id}
                  className="flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-md border border-neutral-200 text-[11px]"
                >
                  <span className="font-medium text-neutral-800 truncate">{doc.title}</span>
                  <span className="text-[9px] text-neutral-400 flex-shrink-0">v{doc.version}</span>
                </li>
              ))}
            </ul>
          </section>
